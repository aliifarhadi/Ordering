    ...
    public sealed class OrderRepository : IOrderRepository
    {
        ...
        private IQueryable<Order> AggregateQuery()
            => _dbContext.Set<Order>()
                ...
                .Include(order => order.OrderServices).ThenInclude(service => service.CoveredServices)
                .Include(order => order.OrderServices).ThenInclude(service => service.CoveredSegments)
                .Include(order => order.OrderServices).ThenInclude(service => service.AirTransportDetail)
                .Include(order => order.OrderServices).ThenInclude(service => service.SeatDetail)
                .Include(order => order.OrderServices).ThenInclude(service => service.BaggageDetail)
                .Include(order => order.OrderServices).ThenInclude(service => service.MealDetail)
                .Include(order => order.OrderServices).ThenInclude(service => service.LoungeDetail)
                ...
                .Include(order => order.FareConstructions).ThenInclude(construction => construction.PricingGroups).ThenInclude(group => group.Travellers)
                .Include(order => order.FareConstructions).ThenInclude(construction => construction.PricingGroups).ThenInclude(group => group.PricingUnits).ThenInclude(unit => unit.FareComponents).ThenInclude(component => component.Services)
                .Include(order => order.FareConstructions).ThenInclude(construction => construction.PricingGroups).ThenInclude(group => group.PricingUnits).ThenInclude(unit => unit.FareComponents).ThenInclude(component => component.Segments)
