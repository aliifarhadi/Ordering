...
|---|---|---|
| **P3-B** | Pre-ticket Cancel / Service Removal | No document, no refund, no provider money. Lowest risk place to unify the rail and land the contract extension |
...
## P3-B — Pre-ticket Cancel / Service Removal

**Deliver**
- Scoped service/item removal (not only whole-order withdrawal), under `ServicingOperationKind.ServiceRemoval`.
...
- Single append-only `ServicingOperationKind` extension: `Refund, Exchange, Revalidate, VoidRefund, ServiceRemoval`.
- Correct the `PricingSource` provenance on locally derived reversals (design §9.1, finding **B-2**).
- Cancellation fee accepted from the pricing owner as a `Fee`/`Penalty` line — never computed.

**Do not**
- Touch documents. Cancel with an issued document for the scope must be refused and routed to Void or Refund.
- Extend `ReverseServiceValue` toward refund semantics.
- Renumber any existing enum value.

**Exit gate** — dependent-service scope respected; capacity released under a stable operation key; a scope with an
issued document is refused; one `PriceChangeSet` per accepted cancellation; replay is idempotent; legacy cancel
rail no longer reachable from a production channel; full regression green.
...
- `VoidRefund` corrective reversal as a **new** operation with its own provider evidence.
- Servicing-record redisplay through the existing projection.

**Do not**
- Reuse `ReverseServiceValue` for refund lines (design §9.1 — the critical boundary).
...
`SettlementOnly` and never moves customer balance; corrective reversal appends and never deletes; exactly one
`OrderPricingChanged` per committed set.
...
8. Corrective operations append; they never delete history or flip status.
